...
void  ParticleSet::deposit(RectMDArray<double>& a_Charge, vector<Particle>& t_particles)
{
  ...
  for (int it = 0; it <t_particles.size(); it++)
    {
      //cout<<"Particle = "<< it<<endl; 
      array<int,DIM> iposLow, iposHigh;
      for (int l = 0; l < DIM; l++)
        {
	  pos[l] = t_particles[it].m_x[l];
	  iposLow[l] = floor(pos[l]/(m_dx*m_L));
	  iposHigh[l] = ceil(pos[l]/(m_dx*m_L)); 
	}
      ...
      for (int j =0; j<DIM; j++)
	{
      ...
      //cout<<"Interpcoeff = "<< interpcoeff<<endl;
	  ...
	  for (int j =0; j<DIM; j++)
	    {
	      ...
	      double val = abs(s[j]*m_dx*m_L - pos[j])/(m_dx*m_L);
	      //cout<<"s[j] = "<<s[j]<<endl;
	      //cout<<"region = "<<region<<endl;
	      //cout<<"particle position = "<< t_particles[it].m_x[j]<< endl;
	      //cout<<"node position = "<< s[j]*m_dx*m_L<<endl;
	      //cout<<"val = "<<val<<endl;
	      KernelProduct *= m_W.apply(val, region);
	    }
	  //cout<<"Charge = "<< interpcoeff*KernelProduct <<endl;
...
void ParticleSet::wrapParticles(vector<Particle>& t_particles)
{
  ...
  for (int j=0; j<DIM; j++)
    {  
      highBoundary[j] = (m_box.getHighCorner()[j]+1)*m_dx*m_L;
      lowBoundary[j] = (m_box.getLowCorner()[j])*m_dx*m_L;
