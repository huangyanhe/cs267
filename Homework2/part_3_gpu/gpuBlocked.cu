...
{    
    ...
    //
    // Need to convert vectors of vectors to pointers
    //
    int numBins = numCells*numCells;
    particle_t *individualBin[maxNumParticlesinBin];
    int *pointerToBins;
    //Pointer to Bins
    cudaMalloc( (void**) &pointerToBins, numBins*sizeof(*individualBin) );
    int *neighborList[9];
    int *pointertoNeighborList;
    //Pointer to NeighborList
    cudaMalloc( (void**) &pointertoNeighborList, numBins*sizeof(*neighborList) );
    for (int j=0; j < numBins; j++)
      {
	 cudaMemcpy( , particles, n * sizeof(particle_t), cudaMemcpyHostToDevice);
      }
